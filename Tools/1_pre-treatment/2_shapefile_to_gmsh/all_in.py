...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec  8 09:48:57 2022
...
def generate_dassflow_mesh_from_sig(bin_dir, write_intermediate_mesh_dir, 
                           shapefile_dir, name_major_bed_shapefile,  mesh_size_major,
                           include_minor_bed =None, name_minor_bed_shapefile =None, mesh_size_minor =None):
    
    # concatenate input to prepare paths 
    path_major_bed = f"{shapefile_dir}/{name_major_bed_shapefile}"
    if name_minor_bed_shapefile is not None:
        path_minor_bed = f"{shapefile_dir}/{name_major_bed_shapefile}"
    
    
    print(">>> INITIALISATION: build raw dassflow mesh from shapefiles function parameters:")
    print("- source shapefile")
...
#    plt.scatter(point_mineur[:,0],point_mineur[:,1])
    ...
    gmsh.write(f"{write_intermediate_mesh_dir}/gmsh_mesh.msh")


    print("- Traduce gmsh mesh to raw DASSLFOW.geo mesh")
    ...
    print(" - Save dassflow.geo mesh")
    
    gen_empty_mesh_dassflow(nodes = nodes, cells = cells, path = f"{write_intermediate_mesh_dir}/dassflow_mesh.geo")
    gen_empty_mesh_dassflow(nodes = nodes, cells = cells, path = f"{bin_dir}/dassflow_mesh_default.geo")
    
    gmsh.finalize()    
    
generate_dassflow_mesh_from_sig(bin_dir =  "/home/livillenave/Documents/distant/dassflow2d-wrap/code/bin_A/", 
                                write_intermediate_mesh_dir =  "/home/livillenave/Documents/distant/dassflow2d-wrap/code/bin_A/mesh_generation", 
                                shapefile_dir = "/home/livillenave/Documents/distant/SD-FLOOD/real_case-AUDE/DATA/DASSFLOW/V2/hydraulic/tmp/", 
                                name_major_bed_shapefile  = "contour1_v2.shp",  
                                mesh_size_major = 200,
                                include_minor_bed =True, 
                                name_minor_bed_shapefile ="contour2.shp", 
                                mesh_size_minor = 100)