...
class runmin():
	def __init__(self, model, init_values):
		...
		self.model = model
		self.dim_all = dassflow2d.wrapping.m_adjoint.get_dim_all()
	...
	def __call__(self, x):
		...
		cost = dassflow2d.wrapping.call_model.func(self = self.model, ctrl_in = x, grad_func = self.jac)  

		#print(nite)
		newj = dassflow2d.wrapping.m_adjoint.get_cost()
		...
		control = dassflow2d.wrapping.m_adjoint.get_array_control()
...
opts = {"disp":True, \
		"gtol" : dassflow2d.wrapping.m_common.get_eps_min(), \
		"maxfun": dassflow2d.wrapping.m_common.get_max_nt_for_adjoint(), \
...
init_values = dassflow2d.wrapping.m_adjoint.get_array_control()
