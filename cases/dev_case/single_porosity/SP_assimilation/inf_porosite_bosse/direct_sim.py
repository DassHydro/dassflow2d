...
my_model.kernel.my_porosity.land[:] = range(1,nc+1)

def func_porosity(x,mil) :
    return 1.*((x<=mil*0.5) or (x>=3*mil*0.5)) + (8./(mil*2)**2*(x-mil*0.5)*(x-mil*0.5*3)+1)*((x>mil*0.5) and (x<3*mil*0.5))

for i in range(0,nc):

    my_model.kernel.my_porosity.phi[i] = func_porosity(i,mil)

#plt.plot(range(0,nc),my_model.kernel.my_porosity.phi)
#plt.title("Porosité imposée")
#plt.show()
