...
a = ( phi0 - phim ) / ( L0 * L0 )
b = -2 * a * xm
c = phi0 + a * ( xm * xm - L0 * L0 )

def phi(x) :

    return (a*x*x+b*x+c)*(abs(x-xm)<=L0)+(phi0)*(abs(x-xm)>L0)


for i in range(nc):

    my_model.kernel.my_porosity.phi[i] = phi(i)
...
print(affichage(graphe,mesh,nc,ts,h,u,v,h0))
