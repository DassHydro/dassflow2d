...
subroutine f90wrap_m_common__weights_initialise(this)
    use m_common, only: weights
    implicit none
...
end subroutine f90wrap_m_common__weights_initialise

subroutine f90wrap_m_common__weights_finalise(this)
    use m_common, only: weights
    implicit none
...
end subroutine f90wrap_m_common__weights_finalise

subroutine f90wrap_m_common__test_dt_nearest(ret_test_dt_nearest, dt_to_test)
...
end subroutine f90wrap_m_common__test_dt_nearest

subroutine f90wrap_m_common__test_dt_just_after(ret_test_dt_just_after, dt_to_test)
...
end subroutine f90wrap_m_common__test_dt_just_after

subroutine f90wrap_m_common__reading_args
    use m_common, only: reading_args
    implicit none
    
    call reading_args()
end subroutine f90wrap_m_common__reading_args

subroutine f90wrap_m_common__machine_number_limits
    use m_common, only: machine_number_limits
    implicit none
    
    call Machine_Number_Limits()
end subroutine f90wrap_m_common__machine_number_limits

subroutine f90wrap_m_common__swap_r(a, b)
    ...
    call swap_r(a=a, b=b)
end subroutine f90wrap_m_common__swap_r

subroutine f90wrap_m_common__swap_i(a, b)
    ...
    call swap_i(a=a, b=b)
end subroutine f90wrap_m_common__swap_i

subroutine f90wrap_m_common__swap_vec_r(vec, swap, n0, n1)
...
end subroutine f90wrap_m_common__swap_vec_r

subroutine f90wrap_m_common__swap_vec_i(vec, swap, n0, n1)
...
end subroutine f90wrap_m_common__swap_vec_i

subroutine f90wrap_m_common__div_by_except_0(a, ret_div_by_except_0, b)
...
end subroutine f90wrap_m_common__div_by_except_0

subroutine f90wrap_m_common__i4col_sort_a(m, n, a, n0, n1)
...
end subroutine f90wrap_m_common__i4col_sort_a

subroutine f90wrap_m_common__sort_heap_external(n, indx, i, j, isgn)
...
end subroutine f90wrap_m_common__sort_heap_external

subroutine f90wrap_m_common__i4col_swap(m, n, a, i, j, n0, n1)
...
end subroutine f90wrap_m_common__i4col_swap

subroutine f90wrap_m_common__i4col_compare(m, n, a, i, j, isgn, n0, n1)
...
end subroutine f90wrap_m_common__i4col_compare

subroutine f90wrap_m_common__file_name_ext(file_name, ret_file_name_res, typ)
...
end subroutine f90wrap_m_common__file_name_ext

subroutine f90wrap_m_common__count_lines(ret_nb_lines, file_name)
...
end subroutine f90wrap_m_common__count_lines
...
subroutine f90wrap_m_common__get__do_warmup(f90wrap_do_warmup)
    use m_common, only: m_common_do_warmup => do_warmup
    implicit none
    logical, intent(out) :: f90wrap_do_warmup
    
    f90wrap_do_warmup = m_common_do_warmup
end subroutine f90wrap_m_common__get__do_warmup

subroutine f90wrap_m_common__set__do_warmup(f90wrap_do_warmup)
    use m_common, only: m_common_do_warmup => do_warmup
    implicit none
    logical, intent(in) :: f90wrap_do_warmup
    
    m_common_do_warmup = f90wrap_do_warmup
end subroutine f90wrap_m_common__set__do_warmup
...
subroutine f90wrap_m_common__get__use_zobs(f90wrap_use_Zobs)
    use m_common, only: m_common_use_Zobs => use_Zobs
    implicit none
    integer(4), intent(out) :: f90wrap_use_Zobs
    
    f90wrap_use_Zobs = m_common_use_Zobs
end subroutine f90wrap_m_common__get__use_zobs

subroutine f90wrap_m_common__set__use_zobs(f90wrap_use_Zobs)
    use m_common, only: m_common_use_Zobs => use_Zobs
    implicit none
    integer(4), intent(in) :: f90wrap_use_Zobs
    
    m_common_use_Zobs = f90wrap_use_Zobs
end subroutine f90wrap_m_common__set__use_zobs

subroutine f90wrap_m_common__get__use_hobs(f90wrap_use_hobs)
    use m_common, only: m_common_use_hobs => use_hobs
    implicit none
    integer(4), intent(out) :: f90wrap_use_hobs
    
    f90wrap_use_hobs = m_common_use_hobs
end subroutine f90wrap_m_common__get__use_hobs

subroutine f90wrap_m_common__set__use_hobs(f90wrap_use_hobs)
    use m_common, only: m_common_use_hobs => use_hobs
    implicit none
    integer(4), intent(in) :: f90wrap_use_hobs
    
    m_common_use_hobs = f90wrap_use_hobs
end subroutine f90wrap_m_common__set__use_hobs

subroutine f90wrap_m_common__get__use_uvobs(f90wrap_use_UVobs)
    use m_common, only: m_common_use_UVobs => use_UVobs
    implicit none
    integer(4), intent(out) :: f90wrap_use_UVobs
    
    f90wrap_use_UVobs = m_common_use_UVobs
end subroutine f90wrap_m_common__get__use_uvobs

subroutine f90wrap_m_common__set__use_uvobs(f90wrap_use_UVobs)
    use m_common, only: m_common_use_UVobs => use_UVobs
    implicit none
    integer(4), intent(in) :: f90wrap_use_UVobs
    
    m_common_use_UVobs = f90wrap_use_UVobs
end subroutine f90wrap_m_common__set__use_uvobs

subroutine f90wrap_m_common__get__use_huvobs(f90wrap_use_HUVobs)
    use m_common, only: m_common_use_HUVobs => use_HUVobs
    implicit none
    integer(4), intent(out) :: f90wrap_use_HUVobs
    
    f90wrap_use_HUVobs = m_common_use_HUVobs
end subroutine f90wrap_m_common__get__use_huvobs

subroutine f90wrap_m_common__set__use_huvobs(f90wrap_use_HUVobs)
    use m_common, only: m_common_use_HUVobs => use_HUVobs
    implicit none
    integer(4), intent(in) :: f90wrap_use_HUVobs
    
    m_common_use_HUVobs = f90wrap_use_HUVobs
end subroutine f90wrap_m_common__set__use_huvobs

subroutine f90wrap_m_common__get__use_qobs(f90wrap_use_Qobs)
    use m_common, only: m_common_use_Qobs => use_Qobs
    implicit none
    integer(4), intent(out) :: f90wrap_use_Qobs
    
    f90wrap_use_Qobs = m_common_use_Qobs
end subroutine f90wrap_m_common__get__use_qobs

subroutine f90wrap_m_common__set__use_qobs(f90wrap_use_Qobs)
    use m_common, only: m_common_use_Qobs => use_Qobs
    implicit none
    integer(4), intent(in) :: f90wrap_use_Qobs
    
    m_common_use_Qobs = f90wrap_use_Qobs
end subroutine f90wrap_m_common__set__use_qobs

subroutine f90wrap_m_common__get__use_qobs_gr4(f90wrap_use_Qobs_gr4)
    use m_common, only: m_common_use_Qobs_gr4 => use_Qobs_gr4
    implicit none
    integer(4), intent(out) :: f90wrap_use_Qobs_gr4
    
    f90wrap_use_Qobs_gr4 = m_common_use_Qobs_gr4
end subroutine f90wrap_m_common__get__use_qobs_gr4

subroutine f90wrap_m_common__set__use_qobs_gr4(f90wrap_use_Qobs_gr4)
    use m_common, only: m_common_use_Qobs_gr4 => use_Qobs_gr4
    implicit none
    integer(4), intent(in) :: f90wrap_use_Qobs_gr4
    
    m_common_use_Qobs_gr4 = f90wrap_use_Qobs_gr4
end subroutine f90wrap_m_common__set__use_qobs_gr4

subroutine f90wrap_m_common__get__use_nse(f90wrap_use_NSE)
    use m_common, only: m_common_use_NSE => use_NSE
    implicit none
    integer(4), intent(out) :: f90wrap_use_NSE
    
    f90wrap_use_NSE = m_common_use_NSE
end subroutine f90wrap_m_common__get__use_nse

subroutine f90wrap_m_common__set__use_nse(f90wrap_use_NSE)
    use m_common, only: m_common_use_NSE => use_NSE
    implicit none
    integer(4), intent(in) :: f90wrap_use_NSE
    
    m_common_use_NSE = f90wrap_use_NSE
end subroutine f90wrap_m_common__set__use_nse

subroutine f90wrap_m_common__get__use_xsshp(f90wrap_use_xsshp)
    use m_common, only: m_common_use_xsshp => use_xsshp
    implicit none
    integer(4), intent(out) :: f90wrap_use_xsshp
    
    f90wrap_use_xsshp = m_common_use_xsshp
end subroutine f90wrap_m_common__get__use_xsshp

subroutine f90wrap_m_common__set__use_xsshp(f90wrap_use_xsshp)
    use m_common, only: m_common_use_xsshp => use_xsshp
    implicit none
    integer(4), intent(in) :: f90wrap_use_xsshp
    
    m_common_use_xsshp = f90wrap_use_xsshp
end subroutine f90wrap_m_common__set__use_xsshp

subroutine f90wrap_m_common__get__xsshp_along_x(f90wrap_xsshp_along_x)
    use m_common, only: m_common_xsshp_along_x => xsshp_along_x
    implicit none
    integer(4), intent(out) :: f90wrap_xsshp_along_x
    
    f90wrap_xsshp_along_x = m_common_xsshp_along_x
end subroutine f90wrap_m_common__get__xsshp_along_x

subroutine f90wrap_m_common__set__xsshp_along_x(f90wrap_xsshp_along_x)
    use m_common, only: m_common_xsshp_along_x => xsshp_along_x
    implicit none
    integer(4), intent(in) :: f90wrap_xsshp_along_x
    
    m_common_xsshp_along_x = f90wrap_xsshp_along_x
end subroutine f90wrap_m_common__set__xsshp_along_x

subroutine f90wrap_m_common__get__xsshp_along_y(f90wrap_xsshp_along_y)
    use m_common, only: m_common_xsshp_along_y => xsshp_along_y
    implicit none
    integer(4), intent(out) :: f90wrap_xsshp_along_y
    
    f90wrap_xsshp_along_y = m_common_xsshp_along_y
end subroutine f90wrap_m_common__get__xsshp_along_y

subroutine f90wrap_m_common__set__xsshp_along_y(f90wrap_xsshp_along_y)
    use m_common, only: m_common_xsshp_along_y => xsshp_along_y
    implicit none
    integer(4), intent(in) :: f90wrap_xsshp_along_y
    
    m_common_xsshp_along_y = f90wrap_xsshp_along_y
end subroutine f90wrap_m_common__set__xsshp_along_y

subroutine f90wrap_m_common__get__use_ptf(f90wrap_use_ptf)
    use m_common, only: m_common_use_ptf => use_ptf
    implicit none
    integer(4), intent(out) :: f90wrap_use_ptf
    
    f90wrap_use_ptf = m_common_use_ptf
end subroutine f90wrap_m_common__get__use_ptf

subroutine f90wrap_m_common__set__use_ptf(f90wrap_use_ptf)
    use m_common, only: m_common_use_ptf => use_ptf
    implicit none
