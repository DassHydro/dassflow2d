...
subroutine f90wrap_m_model__array__porosity_history(dummy_this, nd, dtype, dshape, dloc)
    use m_common
    use m_linear_algebra
    use m_mesh
    use m_mpi
    use m_time_screen
    use m_model, only: m_model_porosity_history => porosity_history
    use, intrinsic :: iso_c_binding, only : c_int
    implicit none
    integer, intent(in) :: dummy_this(2)
    integer(c_int), intent(out) :: nd
    ...
    nd = 2
    dtype = 12
    if (allocated(m_model_porosity_history)) then
        dshape(1:2) = shape(m_model_porosity_history)
        dloc = loc(m_model_porosity_history)
    else
        dloc = 0
    end if
end subroutine f90wrap_m_model__array__porosity_history
