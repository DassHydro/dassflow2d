...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_applyprec( N,f,X,a,ja,ia)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    CALL cagmg_prec_matv(1,S,X,S(N+1),S(2*N+1),.FALSE.)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    !
    !
    USE cagmg_mem
    IMPLICIT NONE
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       CALL cagmg_prec_matv(1,fsc,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    COMPLEX(kind(0.0e0)) ::  f(N), X(N)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmg_prec_matv(l,B,X,AX,R,matv)
    USE cagmg_mem
    IMPLICIT NONE
    INTEGER :: l
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL cagmg_MUMPSseq(n,fff,1,a,ja,ia)
       IF (wfo) THEN
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL cagmg_MUMPSseq(n,fff,1,dt(l)%a,dt(l)%ja,dt(l)%ia)
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmg_MUMPSseq(n,f,ijob,a,ja,ia)
    USE cagmg_mem
    IMPLICIT NONE
!!!!!!!
!
...
!
    ...
    IF (preprocessed) THEN
       CALL cagmg_relmem
       IF (.NOT.allzero)                        &
               CALL cagmg_MUMPSseq(nn(nlev),fdum,-2)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0e0
       cputm=0.0e0
...
310 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL cagmg_applyprec(n,f,x,a,ja,ia)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL cagmg_GCR(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    ELSE
   !
       CALL cagmg_FlexCG(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    END IF
!
