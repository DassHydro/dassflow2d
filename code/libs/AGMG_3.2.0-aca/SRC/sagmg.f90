...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmg_applyprec( N,f,X,a,ja,ia)
    USE sagmg_mem
    IMPLICIT NONE
    INTEGER       :: N
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmg_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    !
    !
    USE sagmg_mem
    IMPLICIT NONE
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       !
       CALL sagmg_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
       IF ( ITER > 1 ) THEN
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmg_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    USE sagmg_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    REAL(kind(0.0e0)) ::  f(N), X(N)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       !
       CALL sagmg_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,.FALSE.)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE sagmg_prec_matv(l,B,X,AX,R,matv)
    USE sagmg_mem
    IMPLICIT NONE
    INTEGER :: l
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL sagmg_MUMPSseq(n,fff,1,a,ja,ia)
       IF (wfo) THEN
...
!
    ...
    ELSE
       ...
       memi=memi-(n+1)
       CALL sagmg_MUMPSseq(n,fff,1,dt(l)%a,dt(l)%ja,dt(l)%ia)
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmg_MUMPSseq(n,f,ijob,a,ja,ia)
    USE sagmg_mem
    IMPLICIT NONE
!!!!!!!
!
...
!
    ...
    IF (preprocessed) THEN
       CALL sagmg_relmem
       IF (.NOT.allzero)                        &
               CALL sagmg_MUMPSseq(nn(nlev),fdum,-2)
       preprocessed=.FALSE.
       solve=.FALSE.
       eltm=0.0e0
       cputm=0.0e0
...
310 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL sagmg_applyprec(n,f,x,a,ja,ia)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL sagmg_GCR(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    ELSE
   !
       CALL sagmg_FlexCG(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    END IF
!
