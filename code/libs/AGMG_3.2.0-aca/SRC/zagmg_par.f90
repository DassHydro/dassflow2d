...
!
    ...
    COMPLEX(kind(0.0d0)), ALLOCATABLE :: buffi(:),buffo(:),vectc(:)
    COMPLEX(kind(0.0d0)), ALLOCATABLE :: avectc(:,:),Sco(:,:),Rco(:,:),xco(:),xcoloc(:)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE zagmgpar_applyprec( N,f,X,a,ja,ia)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    CALL zagmgpar_prec_matv(1,S,X,S(N+1),S(2*N+1),.FALSE.)
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmgpar_diagscal(n,a,ja,ia,idiag,as)
    ...
    END DO
    CALL zagmgpar_sendrec(scald,idum,-1,lstoutL1,ilstoutL1,ilstinL1)
    DO i=1,n
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE zagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    !
    !
    USE zagmgpar_mem
    IMPLICIT NONE
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       CALL zagmgpar_prec_matv(1,fsc,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
...
!-----------------------------------------------------------------------
  SUBROUTINE zagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    COMPLEX(kind(0.0d0)) ::  f(N), X(N)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE zagmgpar_prec_matv(l,B,X,AX,R,matv)
    USE zagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          CALL zagmgpar_MUMPSpar(nnext,R,2)
          !
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          AXN=BN+nnext
...
!
    ...
    ELSE
       ...
       CALL zagmgpar_restorelstrank(ilstinL1,listrank)
       ifl=n+1
       !
...
!
    ...
    CALL zagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,iextL1)
    dt(1)%iext => iextL1
    DEALLOCATE(w,iw)
    !
      CALL zagmgpar_setsendrecL1(n,ja,ia,iextL1,listrank,ifirstlistrank)
      dt(1)%lstin   => lstinL1
...
310 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL zagmgpar_applyprec(n,f,x,a,ja,ia)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL zagmgpar_GCR(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    ELSE
   !
       CALL zagmgpar_FlexCG(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    END IF
!
