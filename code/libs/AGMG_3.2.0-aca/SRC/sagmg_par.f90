...
!
    ...
    REAL(kind(0.0e0)), ALLOCATABLE :: buffi(:),buffo(:),vectc(:)
    REAL(kind(0.0e0)), ALLOCATABLE :: avectc(:,:),Sco(:,:),Rco(:,:),xco(:),xcoloc(:)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_applyprec( N,f,X,a,ja,ia)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    !
    !
    USE sagmgpar_mem
    IMPLICIT NONE
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       !
       CALL sagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
       IF ( ITER > 1 ) THEN
...
!-----------------------------------------------------------------------
  SUBROUTINE sagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    REAL(kind(0.0e0)) ::  f(N), X(N)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       !
       CALL sagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,.FALSE.)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE sagmgpar_prec_matv(l,B,X,AX,R,matv)
    USE sagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          CALL sagmgpar_MUMPSpar(nnext,R,2)
          !
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          AXN=BN+nnext
...
!
    ...
    ELSE
       ...
       CALL sagmgpar_restorelstrank(ilstinL1,listrank)
       ifl=n+1
       !
...
!
    ...
    CALL sagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,iextL1)
    dt(1)%iext => iextL1
    DEALLOCATE(w,iw)
    !
      CALL sagmgpar_setsendrecL1(n,ja,ia,iextL1,listrank,ifirstlistrank)
      dt(1)%lstin   => lstinL1
...
310 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL sagmgpar_applyprec(n,f,x,a,ja,ia)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL sagmgpar_GCR(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    ELSE
   !
       CALL sagmgpar_FlexCG(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    END IF
!
