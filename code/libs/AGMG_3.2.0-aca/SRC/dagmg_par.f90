...
!
    ...
    REAL(kind(0.0d0)), ALLOCATABLE :: buffi(:),buffo(:),vectc(:)
    REAL(kind(0.0d0)), ALLOCATABLE :: avectc(:,:),Sco(:,:),Rco(:,:),xco(:),xcoloc(:)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_applyprec( N,f,X,a,ja,ia)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       !
       !
       CALL dagmgpar_prec_matv(1,f,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
       IF ( ITER > 1 ) THEN
...
!-----------------------------------------------------------------------
  SUBROUTINE dagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID2 > TOL2BNORM2 )
       ...
       !
       !
       CALL dagmgpar_prec_matv(1,f,Su(1+itm*N),Sc(1+itm*N),R,.FALSE.)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE dagmgpar_prec_matv(l,B,X,AX,R,matv)
    USE dagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL dagmgpar_applyprec(n,f,x,a,ja,ia)
