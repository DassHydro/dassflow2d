...
!
    ...
    REAL(kind(0.0e0)) :: memi=0.0,memax=0.0,memr=0.0,mritr,rlenilen,smoothtp,omeg
    REAL(kind(0.0e0)) :: wcplex(4),fracnz(maxlev),flop=0.0e0
    ...
    COMPLEX(kind(0.0e0)), ALLOCATABLE :: buffi(:),buffo(:),vectc(:)
    COMPLEX(kind(0.0e0)), ALLOCATABLE :: avectc(:,:),Sco(:,:),Rco(:,:),xco(:),xcoloc(:)
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_applyprec( N,f,X,a,ja,ia)
    USE cagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N
    ...
    CALL cagmgpar_prec_matv(1,S,X,S(N+1),S(2*N+1),.FALSE.)
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_diagscal(n,a,ja,ia,idiag,as)
    ...
    END DO
    CALL cagmgpar_sendrec(scald,idum,-1,lstoutL1,ilstoutL1,ilstinL1)
    DO i=1,n
...
!!!!!!!!!!!!!!!!!!
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_FlexCG(N,f,X,ITER,RESID,a,ja,ia,init)
    !
    !
    USE cagmgpar_mem
    IMPLICIT NONE
    ...
    DO WHILE ( ITER < MAXIT .AND. RESID > TOL )
       ...
       CALL cagmgpar_prec_matv(1,fsc,S(1+3*N),S(1+4*N),S(1+5*N),.FALSE.)
...
!-----------------------------------------------------------------------
  SUBROUTINE cagmgpar_GCR(N,f,X,ITER,RESID,a,ja,ia,init)
    USE cagmgpar_mem
    IMPLICIT NONE
    INTEGER       :: N, ITER, init
    COMPLEX(kind(0.0e0)) ::  f(N), X(N)
...
!-----------------------------------------------------------------------
  RECURSIVE SUBROUTINE cagmgpar_prec_matv(l,B,X,AX,R,matv)
    USE cagmgpar_mem
    IMPLICIT NONE
    INTEGER :: l
    ...
    IF (nnext > 0) THEN
       ...
       IF (l+1 == nlev) THEN
          !
          CALL cagmgpar_MUMPSpar(nnext,R,2)
          !
       ELSE IF ( innermax(l+1) <= 1 ) THEN
          !
          AXN=BN+nnext
...
!
    ...
    ELSE
       ...
       IF (wfo) THEN
          WRITE(iout,911) IRANK,flop/real(2*nlc1(2))
       END IF
       wcplex(4)=0.0e0
       nwrkcum=1
       IF (ngl(1) > maxcoarset)   wcplex(4)=-1.0e0
       CALL cagmgpar_restorelstrank(ilstinL1,listrank)
       ifl=n+1
       !
    ...
    ELSE IF (ijob == 1) THEN
       ...
       IF (n > 0) THEN
          ...
          ALLOCATE(nloc(NPROC),mumps_par%IRN_loc(mumps_par%NZ_loc),   &
                ...
                mumps_par%A_loc(mumps_par%NZ_loc) )
...
!
    INTEGER :: nza
    REAL(kind(0.0e0)), SAVE :: cputm=0.0e0,eltm=0.0e0,memh
    LOGICAL, SAVE :: preprocessed=.FALSE.,solve=.FALSE.
    INTEGER :: i,init,ijb,k
    REAL(kind(0.0e0)) :: cputmp,eltmp
...
!
    ...
    CALL cagmgpar_partroword(n,a,ja,ia,dt(1)%idiag,w,iw,iextL1)
    dt(1)%iext => iextL1
    DEALLOCATE(w,iw)
    !
      CALL cagmgpar_setsendrecL1(n,ja,ia,iextL1,listrank,ifirstlistrank)
      dt(1)%lstin   => lstinL1
...
310 CONTINUE
   ...
   !
    IF (MOD(ijb,10) >= 3) THEN
       IF (wfo) THEN
          WRITE(iout,901) IRANK
       END IF
       CALL cagmgpar_applyprec(n,f,x,a,ja,ia)
   ...
   !
    ELSE IF (nrst > 1) THEN
   !
       CALL cagmgpar_GCR(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    ELSE
   !
       CALL cagmgpar_FlexCG(n,f,x,iter,resid,a,ja,ia,init)
   !
   !
    END IF
!
