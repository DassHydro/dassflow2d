...
!
!  Differentiation of calc_boundary_state in forward (tangent) mode (with options fixinterface):
...
!                mesh.edge:in
SUBROUTINE CALC_BOUNDARY_STATE_DIFF(mesh, mesh_diff, hl, hl_diff, zl, &
& zl_diff, ul, ul_diff, vl, vl_diff, hr, hr_diff, zr, zr_diff, ur, &
& ur_diff, vr, vr_diff)
! add lilian for linear interp
  ...
  CASE ('zspresc')
    ...
    y1_diff = -bathy_cell_diff(mesh%edge(ie)%cell(1))
    y1 = result1 - bathy_cell(mesh%edge(ie)%cell(1))
    IF (0._rp .LT. y1) THEN
      hr_diff = y1_diff
      hr = y1
    ELSE
    ...
    END IF
    result3 = temp
    y2_diff = ul_diff + two*result1*(result2_diff-result3_diff)
    y2 = ul + two*result1*(result2-result3)
    IF (0._rp .LT. y2) THEN
      ur_diff = y2_diff
      ur = y2
    ELSE
