...
!
!  Differentiation of calc_boundary_state in reverse (adjoint) mode (with options fixinterface):
...
&     ur_back
  ...
  CASE ('zspresc') 
    ...
    END IF
    y2 = ul + two*SQRT(g)*(SQRT(hl)-SQRT(hr))
    IF (0._rp .LT. y2) THEN
      CALL PUSHCONTROL1B(0)
    ELSE
      CALL PUSHCONTROL1B(1)
    ...
    ELSE
      y2_back = 0.0_8
    END IF
    ul_back = ul_back + y2_back
    temp_back = two*SQRT(g)*y2_back
    IF (.NOT.hl .EQ. 0.0) hl_back = hl_back + temp_back/(2.0*SQRT(hl))
    IF (.NOT.hr .EQ. 0.0) hr_back = hr_back - temp_back/(2.0*SQRT(hr))
    CALL POPCONTROL1B(branch)
    IF (branch .EQ. 0) THEN
    ...
    ELSE
      ...
      y1_back = 0.0_8
    END IF
    bathy_cell_back(mesh%edge(ie)%cell(1)) = bathy_cell_back(mesh%edge(&
&     ie)%cell(1)) - y1_back
  CASE ('hpresc') 
    vl_back = vl_back + vr_back
    ul_back = ul_back + ur_back
