...
!
!  Differentiation of euler_time_step_first_b1_porosity in reverse (adjoint) mode (with options fixinterface):
...
SUBROUTINE EULER_TIME_STEP_FIRST_B1_POROSITY_BACK(dof, dof_back, mesh)
...
&         (1)
      ...
      IF (phil .GT. zerom .OR. phir .GT. zerom) THEN
        ...
        CALL SW_HLLC_SP(hl(2), ul(2), vl(2), zl, phil, s2l, hr(2)& ! Replaced by Perl Script
...
&                                       size(bathy_cell,1))
      ...
      IF (branch .EQ. 0) THEN
        ...
        CALL SW_HLLC_SP_BACK(hl(2), hl_back(2), ul(2), ul_back(2), vl(2)&
&                      , vl_back(2), zl, phil, phil_back, s2l, s2l_back&
&                      , hr(2), hr_back(2), ur(2), ur_back(2), vr(2), &
&                      vr_back(2), zr, phir, phir_back, s2r, s2r_back, &
&                      nflux, nflux_back)
      ELSE
...
&       %edge(ie)%normal%y*vl_back(2)
 ...
 110 CALL POPINTEGER4(ir)
    CALL POPINTEGER4(il)
  END DO
END SUBROUTINE EULER_TIME_STEP_FIRST_B1_POROSITY_BACK
