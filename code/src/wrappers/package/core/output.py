...
class Result(object):
    ...
    def plot_field(self, my_mesh = None, my_scalar = None, what="", when = 0, title_plot ="", title_scale_bar="", save_plot=False, filename ="", notebook=False):
        """
        Plot wished spatial variable at wished time, from result object
        ...
        my_mesh: mesh_pyvista object 
                 mesh of the current 'my_model' object, accessible from my_model.meshing.mesh_pyvista
        ...
        title_plot: str, default = "" and receives value of "what",
                    title of the plot
        ...
        Examples
        --------
        Plot norm of velocity at a given time of the current 'my_model' object:
        > u = my_model.outputs.result.u
        > v = my_model.outputs.result.v
        > norm_vel = np.sqrt(u**2+v**2)
        > my_plotter = my_model.outputs.result.plot_field(my_mesh = my_model.meshing.mesh_pyvista,
                                                my_scalar = norm_vel,
        ...
        if(when > len(self.all_time) ):
            print("Warning: time step larger than number of simulated time steps, 'when' should be an integer between 0 and ", len(self.all_time))
            return
        
        if(my_mesh==None):
            print("Warning: no mesh provided to plot_field(), provide argument : my_mesh = my_model.meshing.mesh_pyvista ")
            ...
            if what == "h":
                    my_scalar = self.h[:,time]
            if what == "u":
                    my_scalar = self.u[:,time] 
            if what == "v":
                    my_scalar = self.v[:,time]
            if what == "zs":
                    my_scalar = self.zs[:,time]
            if what == "bathy":
                    my_scalar = self.bathy[:]
            if what == "manning_alpha":
                    my_scalar = self.manning_alpha[:]
            ...
            # mesh plot
            plotter.add_mesh(my_mesh, scalars = my_scalar, scalar_bar_args=sargs,show_edges = False)
            plotter.add_axes(xlabel = "X [m]", ylabel = "Y [m]")
            plotter.view_xy()
            plotter.show_bounds()
            if(title_plot == ""):
                title_plot = what
